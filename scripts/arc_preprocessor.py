...
if __name__ == '__main__':
    ...
    with codecs.open(input_file+'_arc_preprocessed', 'w', 'utf-8') as processed_obj:
        with codecs.open(input_file, 'r', 'utf-8') as in_file_obj:
            for line_rec in in_file_obj:
                line = line_rec.split('\t')[text_column-1]
                processed_obj.write('\t'.join(line_rec.split('\t')[:text_column-1])+'\t'+' '.join(arc_obj.tokenizeRawTweetText(line))+'\t'+'\t'.join(line_rec.split('\t')[text_column:]))
