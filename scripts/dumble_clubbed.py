...
COLOR = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b', '#e377c2',
         '#7f7f7f', '#bcbd22', '#17becf', '#ff81co']

MARKER = ['^', 'o', 'x', 'v', '+', 'D', 's', '8', '<', '>', 's']
#COLOR = []
...
import pandas as pd
my_range = range(1, 11)
#plt.hlines(y=my_range, xmin=0.01, xmax=0.9, color='grey', alpha=0.4)
#plt.scatter(0.4512820512820513, my_range, color='gold', alpha=0.8, label='Original')
...
for key, values in parse_evaluate_log('../logs/'+str(dataset_type)+'/final_'+str(dataset_type)+'.log').items():
    ...
    ordered_df = df.sort_values(by='TYPE')
    print(ordered_df['Obfuscated'])
    my_range = range(1, len(df.index) + 1)
    #plt.hlines(y=my_range, xmin=0.1, xmax=0.6, color='grey', alpha=0.4)
    plt.scatter(ordered_df['Obfuscated'], my_range, color=COLOR[count], marker=MARKER[count], alpha=0.7, label=str(key))
   # plt.scatter(ordered_df['Original'], my_range, color='gold', alpha=0.8, label='Original')
#plt.scatter(ordered_df['Original'], my_range, color='gold', alpha=0.8, label='Original')
plt.axvline(x= round(float(original_val),2), c='gold', label='original')
plt.legend()
plt.yticks(my_range, OBFUSCATED_STRATEGY, rotation=45)
plt.xticks(np.arange(0.01, 0.9, step=0.2))
plt.title(str(model).upper()+" performance on_" +str(dataset_type)+ "_" + str(model) +"_selection", loc='center')
plt.xlabel('F1 (Hate Label)')
plt.ylabel('Obfuscation Type')
#plt.show()
plt.savefig('../visualizations/' +str(dataset_type)+ "_" + str(model) +'.png', dpi=400)
plt.clf()


# commandline statements are
# python dumble_clubbed.py davidson bert 0.4512820512820513
# python dumble_clubbed.py davidson bilstm 0.26198083067092653
# python dumble_clubbed.py davidson svm 0.3190883190883191
# python dumble_clubbed.py waseem bert 0.7869249394673123
# python dumble_clubbed.py waseem bilstm 0.7387606318347508
# python dumble_clubbed.py waseem svm 0.6702127659574468
